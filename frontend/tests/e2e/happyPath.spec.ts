...
// The one full happy path: create watchlist -> add pair via the dropdown -> refresh -> see a
// rate -> create alert -> evaluate. Creates its own watchlist rather than relying on the
// seeded "Travel Fund" data, so this test is self-contained and repeatable.
test("create watchlist, add pair, refresh, see a rate, create alert, evaluate", async ({ page }) => {
  ...
  // Add a currency pair via the dropdowns, never free text.
  ...
  // Evaluate Now - triggered result renders for that row.
