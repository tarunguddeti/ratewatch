...
{
    /// <summary>Condition + positive threshold checked before this method ever runs, via
    /// Condition's type and [Range] on CreateAlertRuleRequest (Api/Requests/AlertRequests.cs);
    /// 404 if the watchlist item doesn't exist. No restriction on multiple rules per item,
    /// including opposing conditions - a second rule on the same pair is just another row.</summary>
    public async Task<AlertRuleDto> CreateAsync(Guid watchlistItemId, AlertCondition condition, decimal threshold, CancellationToken ct)
    {
        ...
        logger.LogInformation("Alert rule {RuleId} created for item {WatchlistItemId}", rule.Id, watchlistItemId);
        return ToDto(rule);
    }
    ...
    /// <summary>Obtains the pair's rate live at the moment it runs (never reads a stored
    /// snapshot, so it never depends on a prior refresh having happened), upserts RateSnapshot
    /// as a side effect, compares against the rule's condition (Above/Below strict,
    /// AboveOrEqual/BelowOrEqual inclusive), and records a trigger event only when the
    /// condition is actually satisfied.</summary>
    public async Task<EvaluateResultDto> EvaluateAsync(Guid ruleId, CancellationToken ct)
    {
        ...
        if (!rateResult.IsSuccess)
        {
            ...
            // Result type itself past this layer.
        ...
        // same rate does count. Exhaustive over AlertCondition's four members, not a
        ...
        if (triggered)
        {
            ...
            logger.LogInformation("Alert rule {RuleId} triggered at rate {Rate}", rule.Id, rateResult.Rate);
        }
