    ...
    ILogger<WatchlistItemService> logger)
{
    ...
    /// paths, each a distinct status code via the exception hierarchy. Currency validation is
    /// two-layer and fail-closed: if the supported-currency list itself can't be verified,
    /// GetSupportedCurrenciesAsync throws RateProviderUnavailableException and this method
    /// never catches it - an unverifiable currency is never treated as valid.</summary>
    public async Task<WatchlistItemDto> AddItemAsync(Guid watchlistId, string baseCurrency, string quoteCurrency, CancellationToken ct)
    {
        ...
        // Well-formed-shape checking happens before this method ever runs, via
        ...
        // A currency cannot be tracked against itself. Load-bearing on v2: the provider itself
        // returns 200/rate 1.0 for a same-currency pair rather than rejecting it, so this guard
        ...
        logger.LogInformation("Watchlist item {ItemId} added to watchlist {WatchlistId} ({Base}/{Quote})", item.Id, watchlistId, normalizedBase, normalizedQuote);
        return new WatchlistItemDto(item.Id, item.WatchlistId, item.BaseCurrency, item.QuoteCurrency);
    }

    /// <summary>Cascades to the pair's alert rules and their recorded events at the database
    /// level. The "this pair has N alert rules" warning is a client-side UX step before this
    /// call is made, using data the detail page already has loaded.</summary>
