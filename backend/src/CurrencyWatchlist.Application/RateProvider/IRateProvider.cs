...
/// Frankfurter directly, under any condition (constitution Article IV).</summary>
public interface IRateProvider
{
    /// <summary>Single pair - used by evaluate. Obtains a live rate at call time, never
    /// reads a stored snapshot (FR-020).</summary>
    Task<RateResult> GetLatestRateAsync(string baseCurrency, string quoteCurrency, CancellationToken ct);

    /// <summary>One base, many quotes - used by refresh's batched-by-base strategy
    /// (docs/architecture.md:547-569). One RateResult per requested quote.</summary>
    ...
    /// <summary>Date-range time series, proxied live - used by the history endpoint
    /// (FR-015). Never reads or writes RateSnapshot.</summary>
    ...
    /// it's a single-outcome call, never looped, so there's nothing for a Result type to
    /// buy here (docs/architecture.md:1053-1059).</summary>
