...
A few things about this shape are worth stating plainly rather than leaving implicit: `WatchlistItem` enforces a uniqueness constraint per watchlist so the same pair can't be added twice; deleting a `Watchlist` cascades all the way down through its items, rules, and events; `Above`/`Below` are strict inequalities (`>` / `<`), with `AboveOrEqual`/`BelowOrEqual` as separate, explicit inclusive variants (`>=` / `<=`) rather than one global choice; all rate and threshold arithmetic uses `decimal`, never `double`, end to end; and multiple alert rules on the same item are intentionally allowed — a two-sided "above X, below Y" pair of alerts is a legitimate setup, not a duplicate to reject. Full reasoning for each of these is under **Decisions & Tradeoffs → Data Model & Business Rules**.
...
**Alert `condition` values:** `Above`, `AboveOrEqual`, `Below`, `BelowOrEqual` — four values, not two. `Above`/`Below` are strict (`rate > threshold` / `rate < threshold`); `AboveOrEqual`/`BelowOrEqual` are their inclusive counterparts (`rate >= threshold` / `rate <= threshold`). A rate exactly at the threshold triggers only the variant the rule was explicitly created with. See Decisions & Tradeoffs → Data Model & Business Rules for why both variants exist side by side rather than picking one globally.
...
sequenceDiagram
    ...
    U->>FE: Pick an item, Condition (Above/AboveOrEqual/Below/BelowOrEqual), Threshold, submit
...
> **Decision:** **Strict and inclusive comparisons are both offered as explicit, separate condition values — `Above`, `AboveOrEqual`, `Below`, `BelowOrEqual` — rather than picking one meaning for `Above`/`Below` globally.** "Goes above 1.60" is genuinely ambiguous: some readers mean strictly past that level, others mean at-or-past it, and picking one interpretation for everyone silently gets it wrong for the other half. Making it an explicit fourth-and-final choice on the condition itself, rather than a single global policy, removes the ambiguity instead of guessing at it. `Above`/`Below` keep their original strict meaning (`rate > threshold` / `rate < threshold`) so a threshold sitting exactly on the line is not treated as a match for either; `AboveOrEqual`/`BelowOrEqual` are the new inclusive counterparts (`rate >= threshold` / `rate <= threshold`) where that same boundary value does count as a trigger. Worth a dedicated set of unit tests asserting `rate == threshold → not triggered` for `Above`/`Below` and `rate == threshold → triggered` for `AboveOrEqual`/`BelowOrEqual`, since that boundary is exactly what a reviewer would check first.
